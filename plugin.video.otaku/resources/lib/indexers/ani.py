...
class ANIAPI:
    def __init__(self):
        ...
        self.baseUrl2 = "https://api.ani.zip/"
    ...
    def _parse_episode_view(self, res, anilist_id, season, poster, fanart, eps_watched, update_time, tvshowtitle,
        ...
        episode_data = res['episodes']['1']  # Get the data for the episode

        url = "%s/%s/" % (anilist_id, episode_data['episodeNumber'])
        ...
        title = episode_data['title'].get('en')  # Get the English title
        if not title:
            title = "Episode %s" % episode_data["episodeNumber"]

        image = self.imagePath % episode_data['image'] if episode_data.get('image') else poster
        show_art = {}
        ...
        info = {
            ...
            'title': episode_data['title']['en'],
            'season': season,
            'episode': int(episode_data['episodeNumber']),
            'tvshowtitle': tvshowtitle,
            'mediatype': 'episode'
        }

        if eps_watched:
            if int(eps_watched) >= episode_data['episodeNumber']:
                info['playcount'] = 1

        aired = ''
        try:
            info['aired'] = episode_data['airDate'][:10]
            aired = episode_data['airDate'][:10]
        except:
            pass

        parsed = utils.allocate_item(title, "play/%s" % url, False, image, info, fanart, poster, cast, landscape, banner, clearart, clearlogo)
        database._update_episode(anilist_id, season, episode_data['episodeNumber'], '', update_time, parsed, air_date=aired)

        if title_disable and info.get('playcount') != 1:
            parsed['info']['title'] = 'Episode %s' % episode_data["episodeNumber"]
